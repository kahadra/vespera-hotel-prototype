...
첫 자동 실행은 제한된 Codex 관리 프로세스 환경에서 Chromium Mojo IPC 생성이 Windows 오류 `0x5`로 차단되어 렌더러 진입 전 `electron.exe`의 `0x80000003` 예외 창을 냈다. 소유한 프로세스 트리를 정리하도록 하네스를 보강하고, 상속된 `ELECTRON_RUN_AS_NODE`·Crashpad 환경을 제거한 뒤 일반 사용자 권한에서 다시 실행했다. 개발·패키지 모두 Electron 보안 sandbox를 유지한 채 PASS했으므로 이 최초 오류는 게임 JavaScript 결함이나 sandbox 비활성화 사유로 분류하지 않는다. 별도 시스템 설치나 `--no-sandbox` 우회도 필요하지 않았다.
