...
function desktopModeHubButton(label = "모드 선택으로 돌아가기") {
  if (globalThis.vesperaDesktop?.platform !== "electron") return "";
  return `<button class="button secondary" data-action="return-mode-hub">${escapeHtml(label)}</button>`;
}

function checkpointPhaseLabel(phase) {
  return {
    TUTORIAL: "배치 연습",
    STORY: "시나리오",
    RELIC_OFFER: "전시품 선택",
    ENDLESS_BRIEFING: "시즌 브리핑",
    ENDLESS_AUDIT: "시즌 감사",
    DAY_OPENING: "영업 개시",
    RESERVATION: "예약",
    PLACEMENT: "객실 배치",
...
export function renderModeHub(app, modes) {
  const cards = modes.map((mode) => {
    const available = mode.status === "AVAILABLE";
    const invalid = mode.status === "INVALID";
    const statusCopy = available
      ? `저장 감지 · ${checkpointPhaseLabel(mode.phase)} · 시드 ${mode.runSeed}`
      : invalid
        ? "저장 데이터 확인 필요 · 모드에서 새 게임 가능"
        : "진행 중인 영업 없음";
    const actionLabel = "모드 열기";
    return `<article class="mode-hub-card ${available ? "has-checkpoint" : invalid ? "invalid-checkpoint" : ""}" data-mode-card="${escapeHtml(mode.id)}"><div class="mode-hub-card-heading"><span class="mode-hub-glyph" aria-hidden="true">${escapeHtml(mode.glyph)}</span><div><p class="eyebrow">${escapeHtml(mode.eyebrow)}</p><h2>${escapeHtml(mode.label)}</h2></div></div><p>${escapeHtml(mode.description)}</p><div class="mode-save-status ${available ? "available" : invalid ? "invalid" : "empty"}"><small>로컬 활성 저장</small><b>${escapeHtml(statusCopy)}</b></div><button class="button ${mode.id === "campaign" ? "primary" : "secondary"} large" data-mode-id="${escapeHtml(mode.id)}">${escapeHtml(actionLabel)}</button></article>`;
  }).join("");
  app.innerHTML = `<div class="app-frame mode-hub-frame"><header class="topbar mode-hub-topbar"><div class="brand-lockup"><span class="brand-mark" aria-hidden="true">◆</span><div><p class="eyebrow">HOTEL VESPERA</p><strong>베스페라 호텔</strong></div></div><div class="phase-stack"><div class="phase-pill">운영 기록 선택</div><span class="showcase-mini">OFFLINE DESKTOP</span></div><div class="mode-hub-storage"><small>저장 권위</small><b>이 PC의 로컬 파일</b></div></header><main class="content-frame mode-hub-content"><section class="mode-hub-screen"><div class="mode-hub-heading"><p class="eyebrow">POSSIBLE WORLDS · OPERATION LEDGERS</p><h1>어떤 영업 기록을 열까요?</h1><p>각 모드는 서로 다른 활성 저장을 사용합니다. 한 모드를 열거나 허브로 돌아와도 다른 모드의 기록은 지워지지 않습니다.</p></div><div class="mode-hub-grid">${cards}</div><p class="mode-hub-note">운영 수첩과 해금 기록은 같은 로컬 프로필에서 공유하며, 골드·시설·객실 상태·관계 진척은 각 실행에 따로 귀속됩니다.</p></section></main></div>`;
}
...
const SYNERGY_SCOPE_PRESENTATION = Object.freeze({
  HORIZONTAL_ADJACENT: Object.freeze({ symbol: "↔", label: "옆방" }),
  VERTICAL_ADJACENT: Object.freeze({ symbol: "↕", label: "위·아래" }),
  SAME_FLOOR_GROUP: Object.freeze({ symbol: "F", label: "같은 층 집단" }),
  SAME_FLOOR: Object.freeze({ symbol: "F", label: "같은 층" }),
});

const FACILITY_RELATION_LABELS = Object.freeze({
  ANY: "완공 객실 중 자유롭게 선택",
  VERTICAL_ADJACENT: "같은 동의 바로 위·아래 객실 선택",
  NON_ADJACENT: "서로 이웃하지 않은 객실 선택",
});

const FACILITY_EFFECT_SCOPE_LABELS = Object.freeze({
  TARGETS: "선택 객실",
  TARGET_PAIR: "선택한 두 객실",
  PAIR_ENDPOINTS: "선택한 두 객실",
  HORIZONTAL_NEIGHBORS: "같은 층 양옆 객실",
});

function synergyScopePresentation(scope) {
  return SYNERGY_SCOPE_PRESENTATION[scope] ?? null;
}

function maximumRoomsPerFloor(data) {
...
function installedFacilitiesAtRoom(board, roomId) {
  return (board.installedFacilities ?? []).filter((installation) =>
    (installation.target_room_ids ?? []).includes(roomId));
}

function roomBonusPresentation(data, bonus) {
  const facilityName = bonus.facility_name
    ?? data.indexes.facilities?.[bonus.facility_id]?.name
    ?? data.indexes.upgrades?.[bonus.facility_id]?.name
    ?? "시설";
  return `${facilityName} · ${bonus.label} ${signed(bonus.points)}`;
}
...
function renderRoom(data, room, board, occupantId, controller, evaluation) {
  ...
  const installations = installedFacilitiesAtRoom(board, room.id);
  const occupyingInstallations = installations.filter((installation) => installation.occupies_target);
  const bonuses = (board.roomBonuses ?? []).filter((bonus) => bonus.room_id === room.id);
  const installationText = installations.map((installation) => `${installation.icon ?? "◇"} ${installation.name}`).join(" · ");
  const occupyingText = occupyingInstallations.map((installation) => installation.name).join(" · ");
  const bonusText = bonuses.map((bonus) => roomBonusPresentation(data, bonus)).join(" · ");
  const markerCount = Number(Boolean(installationText)) + Number(Boolean(bonusText));
  return `
    <article class="${classes.join(" ")}" ${roomAction}>
      ...
      <div class="room-body">
        ${blocked
          ? occupyingInstallations.length
            ? `<div class="facility-in-room"><span>${escapeHtml(occupyingInstallations[0].icon ?? "◇")}</span><b>${escapeHtml(occupyingText)}</b></div>`
      ...
      ${!blocked && markerCount ? `<div class="room-card-markers count-${markerCount}">${installationText ? `<span class="room-facility-marker" title="${escapeHtml(installationText)}">${escapeHtml(installationText)}</span>` : ""}${bonusText ? `<span class="room-bonus" title="${escapeHtml(bonusText)}">${escapeHtml(bonusText)}</span>` : ""}</div>` : ""}
    </article>`;
}
...
function renderGroupEffects(evaluation) {
  ...
  return evaluation.groupEffects.map((effect) => {
    const scope = synergyScopePresentation(effect.scope);
    const effectClass = ["synergy", "conflict"].includes(effect.type) ? effect.type : "effect";
    const typeLabel = effect.type === "synergy" ? "시너지" : effect.type === "conflict" ? "상극" : "효과";
    const scopeTag = scope
      ? `<small class="effect-scope"><i aria-hidden="true">${escapeHtml(scope.symbol)}</i>${escapeHtml(scope.label)}</small>`
      : "";
    return `<span class="effect-chip ${effectClass}"><b>${typeLabel}</b>${scopeTag}<span>${escapeHtml(effect.label)} ${signed(effect.points)}</span></span>`;
  }).join("");
}
...
function renderPlacement(controller) {
  ...
  const roomColumns = maximumRoomsPerFloor(data);
    ...
    <section class="screen-shell placement-screen">
      ...
      <div class="placement-layout">
        <section class="board-panel" data-video-target="hotel-board">
          <div class="hotel-board" data-room-columns="${roomColumns}" style="--room-columns:${roomColumns}">${floorRows}</div>
...
function facilityEffectDescription(effect) {
  const scope = FACILITY_EFFECT_SCOPE_LABELS[effect.scope] ?? "선택 위치 기준";
  const points = Number.isFinite(effect.points) ? ` · 만족도 ${signed(effect.points)}` : "";
  return `${effect.label} · ${scope}${points}`;
}

function renderFacilityInstallationPicker(controller) {
  if (!controller.state.pendingFacilityInstallation) return "";
  const options = controller.facilityInstallationOptions?.();
  if (!options) return "";
  const { data } = controller;
  const board = controller.structuralBoardState();
  const roomColumns = maximumRoomsPerFloor(data);
  const optionByRoom = Object.fromEntries((options.rooms ?? []).map((option) => [option.roomId, option]));
  const affectedRoomIds = new Set(options.preview?.affectedRoomIds ?? []);
  const selectedRoomIds = options.selectedRoomIds ?? [];
  const floors = [...new Set(data.rooms.map((room) => room.floor))].sort((a, b) => b - a);
  const roomRows = floors.map((floor) => {
    const rooms = data.rooms.filter((room) => room.floor === floor).sort((left, right) => left.wing - right.wing);
    const choices = rooms.map((sourceRoom) => {
      const option = optionByRoom[sourceRoom.id] ?? { roomId: sourceRoom.id, selectable: false, reason: "선택할 수 없는 객실" };
      const selected = option.selected ?? selectedRoomIds.includes(sourceRoom.id);
      const affected = affectedRoomIds.has(sourceRoom.id);
      const canSelect = Boolean(selected || option.selectable);
      const room = board.rooms[sourceRoom.id] ?? sourceRoom;
      const attrs = [...(room.attributes ?? [])].map(attributeLabel).join(" · ") || "일반";
      const installed = installedFacilitiesAtRoom(board, sourceRoom.id);
      const installedLabel = installed.map((installation) => installation.name).join(" · ");
      const stateLabel = selected
        ...
        : affected
          ...
          : option.selectable
            ? "선택 가능"
            : option.reason ?? "선택 불가";
      const classes = ["installation-room"];
      if (selected) classes.push("selected");
      if (affected) classes.push("affected");
      if (!canSelect) classes.push("unavailable");
      return `<button type="button" class="${classes.join(" ")}" data-action="select-facility-room" data-room-id="${escapeHtml(sourceRoom.id)}" aria-pressed="${selected ? "true" : "false"}" title="${escapeHtml(option.reason ?? stateLabel)}" ${canSelect ? "" : "disabled"}><span class="installation-room-heading"><b>${escapeHtml(sourceRoom.id)}</b><small>${escapeHtml(attrs)}</small></span><strong>${escapeHtml(stateLabel)}</strong>${installedLabel ? `<em>설치됨 · ${escapeHtml(installedLabel)}</em>` : ""}</button>`;
    }).join("");
    return `<div class="installation-floor"><b>F${floor}</b>${renderElevatorLanding(floor, true)}${choices}</div>`;
  }).join("");
  const effectRows = (options.effects ?? []).map((effect) =>
    `<li><span>${escapeHtml(facilityEffectDescription(effect))}</span></li>`).join("");
  const requiredAttributeText = (options.requiredAttributes ?? [])
    .map((attribute) => `${attribute.label} 객실`)
    .join(" · ");
  const previewTargets = options.preview?.targetRoomIds ?? selectedRoomIds;
  const previewEffects = options.preview?.effectLabels ?? [];
  const relationLabel = FACILITY_RELATION_LABELS[options.relation] ?? "표시된 조건에 맞는 객실 선택";
  const upgrade = data.indexes.upgrades[options.upgradeId];
  return `<div class="facility-installation-overlay" role="dialog" aria-modal="true" aria-label="${escapeHtml(options.name)} 설치 위치 선택"><section class="facility-installation-panel" data-room-columns="${roomColumns}" style="--room-columns:${roomColumns}"><header><span class="installation-facility-icon" aria-hidden="true">${escapeHtml(options.icon ?? "◇")}</span><div><p class="eyebrow">FACILITY INSTALLATION</p><h1>${escapeHtml(options.name)} 설치 위치</h1><p>${options.targetCount}곳 필요 · ${escapeHtml(relationLabel)}${upgrade ? ` · ${upgrade.cost}G` : ""}</p></div><button class="handbook-close" data-action="cancel-facility-installation" aria-label="시설 설치 취소">×</button></header><div class="installation-summary"><section><b>시설 고유 효과</b>${requiredAttributeText ? `<p class="installation-requirement"><span>설치 조건</span><strong>${escapeHtml(requiredAttributeText)}</strong></p>` : ""}<ul>${effectRows || "<li><span>선택 위치에 시설 고유 효과를 적용합니다.</span></li>"}</ul></section><section class="installation-preview"><b>현재 영향 예상</b><p>${previewTargets.length ? `설치 ${escapeHtml(previewTargets.join(" · "))}` : "객실을 선택하세요."}${affectedRoomIds.size ? ` · 영향 ${escapeHtml([...affectedRoomIds].join(" · "))}` : ""}</p>${previewEffects.length ? `<small>${escapeHtml(previewEffects.join(" · "))}</small>` : ""}</section></div><div class="installation-board">${roomRows}</div><footer><p><b>${selectedRoomIds.length} / ${options.targetCount}</b>곳 선택${options.complete ? " · 설치 조건 충족" : " · 조건을 만족하는 객실을 더 선택하세요."}</p><div><button class="button secondary" data-action="cancel-facility-installation">설치 취소</button><button class="button primary" data-action="confirm-facility-installation" ${options.complete ? "" : "disabled"}>${options.complete ? "이 위치에 설치 확정" : `${options.targetCount}곳 선택 필요`}</button></div></footer></section></div>`;
}

function renderRenovationCard(controller, upgrade) {
  const { data, state } = controller;
  const pendingInstallationId = state.pendingFacilityInstallation?.upgradeId ?? null;
  const installationPending = Boolean(pendingInstallationId);
  const selectingInstallation = pendingInstallationId === upgrade.id;
  ...
  const enabled = !installationPending && !purchased && !contractedKind && affordable && buildable;
  ...
  const buttonLabel = purchased
    ...
    : selectingInstallation
      ...
      : upgrade.kind === "EXPANSION"
        ? "증축 계약"
        : "설치 계약";
  return `<article class="facility-card rarity-${upgrade.rarity.toLowerCase()} ${enabled ? "" : "locked"} ${purchased ? "contracted" : ""} ${selectingInstallation ? "selecting-installation" : ""}" style="--rank-color:${escapeHtml(rankOf(data, upgrade.rarity).color)}">
...
function renderUpgrade(controller) {
  ...
  const installationPending = Boolean(state.pendingFacilityInstallation);
  ...
  return `
    <section class="screen-shell shop-screen">
      ...
      <div class="shop-footer"><p>${installationPending ? "시설 설치 위치를 확정하거나 취소해야 다음 영업을 준비할 수 있습니다." : state.pendingStayoverCleaningRequest ? "손님의 청소 요청을 먼저 처리해야 합니다." : "공사업체 제안은 호텔 평판에 따라 달라집니다. 동관은 1층 → 2층 → 3층 순서로 증축합니다."}</p><button class="button ${hasContracts ? "primary" : "secondary"}" data-action="finish-upgrade" ${state.pendingStayoverCleaningRequest || installationPending ? "disabled" : ""}>${hasContracts ? "준비 완료 · 다음 영업" : "공사 없이 다음 영업"}</button></div>
    </section>`;
}
...
function renderSpeciesRulesPage(controller) {
  return `<div class="handbook-page"><div class="handbook-page-heading"><p class="eyebrow">SPECIES ACCOMMODATION</p><h2>종족별 숙박 조건</h2><p>같은 종족도 옆방·위아래·같은 층처럼 서로 다른 공간 배치에서 힘을 얻으며, 상극은 같은 층에서 만족도를 낮춥니다.</p></div><div class="manual-entry-grid">${controller.data.species.map((species) => {
    const locked = !controller.state.seenSpeciesIds.includes(species.id);
    const synergyRows = (species.synergies ?? []).map((synergy) => {
      const scope = synergyScopePresentation(synergy.scope);
      return `<p class="species-synergy-row"><span class="synergy-scope"><i aria-hidden="true">${escapeHtml(scope?.symbol ?? "◇")}</i>${escapeHtml(scope?.label ?? "공간 조건")}</span><b>${escapeHtml(synergy.label)}</b><small>${synergy.minimum_participants}명부터 · 1인당 ${signed(synergy.points_per_guest)}</small></p>`;
    }).join("");
...
function renderFinal(controller) {
  ...
  const recordArchiveLocation = globalThis.vesperaDesktop?.platform === "electron"
    ? "이 PC에 보존된 실행 기록"
    : "이 브라우저에 보존된 실행 기록";
  ...
  const managerOutcome = campaign && record?.manager_outcome
    ...
    : "";
  return `<section class="screen-shell final-screen"><div class="result-hero ${campaign && !isComplete ? "failure" : ""}" data-video-target="showcase-final"><p class="eyebrow">${escapeHtml(heroEyebrow)}${record?.ending_tier ? ` · ${escapeHtml(record.ending_tier)}` : ""}</p><span class="result-glyph">◆</span><h1>${escapeHtml(heroTitle)}</h1><p>${escapeHtml(heroDescription)}</p></div><article class="run-record-card" data-video-target="run-record"><div><small>ENDING · ${escapeHtml(record?.ending_id ?? "UNRESOLVED")}</small><h2>${escapeHtml(record?.title ?? "종료 판정 기록 없음")}</h2></div><span class="status-chip ${isComplete ? "clear" : "warning"}">${isComplete ? (campaign ? "성공" : "완료") : campaign ? "실패" : "미완료"}</span><p>기록 ID <b>${escapeHtml(record?.record_id ?? "-")}</b> · ${recordArchiveLocation} ${controller.state.recordArchiveCount}개</p>${managerOutcome}${relicNames.length ? `<p><b>이번 실행의 전시품</b> — ${escapeHtml(relicNames.join(" · "))}</p>` : ""}</article><div class="final-summary"><article><small>완료 영업</small><strong>${nightResults.length} / ${controller.totalNights}</strong></article><article><small>${incomeLabel}</small><strong>${incomeValue}G</strong></article><article><small>${reputationLabel}</small><strong>${signed(reputationValue)}</strong></article><article><small>시설·증축</small><strong>${ownedUpgradeIds.length}개 · 증축 ${expandedRooms}</strong></article><article><small>실행 시드</small><strong>${controller.state.runSeed}</strong></article></div>${epilogues}${lastResult && !campaign ? resultBreakdown(controller, lastResult) : ""}<div class="center-action split-actions"><button class="button primary large" data-action="restart">${escapeHtml(restartLabel)}</button>${desktopModeHubButton()}</div></section>`;
}
...
export function renderApp(app, controller) {
  ...
  app.innerHTML = `<div class="app-frame">${renderHeader(controller)}<div class="content-frame">${content}</div>${renderHandbook(controller)}${renderReservationBoard(controller)}${renderFacilityInstallationPicker(controller)}</div>`;
}
