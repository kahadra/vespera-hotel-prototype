...
function facilityPlacementRelationValid(facility, roomIds, board) {
  const targetCount = facility.installation?.target_count ?? 0;
  if (roomIds.length !== targetCount || new Set(roomIds).size !== roomIds.length) return false;
  if (targetCount === 1) return facility.installation?.relation === "ANY";
  const [leftId, rightId] = roomIds;
  if (facility.installation?.relation === "VERTICAL_ADJACENT") {
    return areVerticalAdjacent(leftId, rightId, board);
  }
  if (facility.installation?.relation === "NON_ADJACENT") {
    return !areHorizontalAdjacent(leftId, rightId, board)
      && !areVerticalAdjacent(leftId, rightId, board);
  }
  return false;
}

function facilityAffectedRoomIds(facility, targetRoomIds, board) {
  const affected = new Set();
  for (const effect of facility.effects ?? []) {
    if (["ROOM_ATTRIBUTES", "ADJACENCY_LINK"].includes(effect.type)
      || ["TARGETS", "PAIR_ENDPOINTS"].includes(effect.scope)) {
      targetRoomIds.forEach((roomId) => affected.add(roomId));
    } else if (effect.scope === "HORIZONTAL_NEIGHBORS") {
      for (const targetRoomId of targetRoomIds) {
        const target = board.rooms[targetRoomId];
        if (!target) continue;
        for (const room of Object.values(board.rooms)) {
          if (
            board.unlockedRooms.has(room.id)
            && room.floor === target.floor
            && Math.abs(room.wing - target.wing) === 1
...
function formalCampaignOperationPlanAuthority(data) {
  if (data.prototype_mode?.type !== "FORMAL_CAMPAIGN") return null;
  const plan = data.campaign?.operation_plan;
  const progressConfig = data.campaign?.formal_progress;
  if (!plan) throw new Error("FORMAL_CAMPAIGN requires data.campaign.operation_plan");
  if (!progressConfig) throw new Error("FORMAL_CAMPAIGN requires data.campaign.formal_progress");
...
function initialState(data, seed, recordArchiveCount = 0, profile = null) {
  ...
  return {
    ...
    ownedUpgradeIds: [],
    facilityPlacements: {},
    pendingFacilityInstallation: null,
...
export class GameController {
  constructor(data, options = {}) {
    formalCampaignOperationPlanAuthority(data);
    this.data = data;
  ...
  get formalCampaignOperationPlan() {
    return this.isFormalCampaignMode
      ? formalCampaignOperationPlanAuthority(this.data)
      : null;
  }

  resolveFormalCampaignOperation() {
    if (!this.isFormalCampaignMode) return null;
    const progress = this.state.campaignProgress;
    const legacyOperation = campaignOperationDescriptor(
      this.formalCampaignProgressConfig,
      progress,
    );
    const scenarioIds = this.data.scenarios.map((scenario) => scenario.id);
    const planOperation = resolveCampaignOperation(
      this.formalCampaignOperationPlan,
      legacyOperation.stageNumber,
      scenarioIds,
    );
    const scenarioIndex = this.data.scenarios.findIndex(
      (scenario) => scenario.id === planOperation.scenario_id,
    );
    if (scenarioIndex < 0) {
      throw new Error("FORMAL_CAMPAIGN operation plan references an unavailable scenario");
    }
    if (
      planOperation.stage_number !== legacyOperation.stageNumber
      || planOperation.operation_kind !== legacyOperation.operationKind
      || this.data.scenarios[legacyOperation.templateIndex]?.id !== planOperation.scenario_id
    ) {
      throw new Error("FORMAL_CAMPAIGN operation plan conflicts with legacy progress descriptor");
    }
    return { planOperation, legacyOperation, scenarioIndex, scenarioIds };
  }
  ...
  hotelContext() {
    return {
      ...
      facilityPlacements: this.state.facilityPlacements,
  ...
  beginOperatingDay(index) {
    ...
    if (this.isFormalCampaignMode) {
      ...
      const operationAuthority = this.resolveFormalCampaignOperation();
      this.state.currentNightIndex = operationAuthority.scenarioIndex;
  ...
  beginNight(index) {
    let formalPlanOperation = null;
    if (this.isFormalCampaignMode) {
      const operationAuthority = this.resolveFormalCampaignOperation();
      formalPlanOperation = operationAuthority.planOperation;
      if (index !== operationAuthority.scenarioIndex) return false;
    ...
    const stage = this.isFormalCampaignMode
      ? formalPlanOperation.stage_number
      : this.isEndlessMode
    ...
    this.state.pendingFacilityInstallation = null;
  ...
  prepareFormalCampaignCompletion() {
    ...
    const operationAuthority = this.resolveFormalCampaignOperation();
    const operation = operationAuthority.legacyOperation;
    if (operationAuthority.scenarioIndex !== this.state.currentNightIndex) {
      throw new Error("Formal campaign operation does not match the explicit plan scenario");
    }
    ...
    return {
      ...
      planIdentity: campaignPlanOperationIdentity(
        this.formalCampaignOperationPlan,
        operationAuthority.planOperation,
        operationAuthority.scenarioIds,
      ),
      nextProgress,
    };
  }
  ...
  completeNight(result) {
    ...
    try {
      ...
      if (formalCompletion) {
        ...
        resolvedResult.campaignPlanIdentity = clone(formalCompletion.planIdentity);
        const pending = this.state.campaignPendingExpenses;
        const config = this.activeCampaignFinanceConfig;
  ...
  buyUpgrade(upgradeId) {
    ...
    if (this.state.pendingFacilityInstallation) return false;
    ...
    if (upgrade.kind === "FACILITY") {
      this.state.pendingFacilityInstallation = { upgradeId, roomIds: [] };
      const initialOptions = this.facilityInstallationOptions();
      let hasValidPlacement = initialOptions?.rooms.some((room) => room.selectable) ?? false;
      if (hasValidPlacement && initialOptions.targetCount === 2) {
        hasValidPlacement = initialOptions.rooms.some((firstRoom) => {
          if (!firstRoom.selectable) return false;
          this.state.pendingFacilityInstallation.roomIds = [firstRoom.roomId];
          return this.facilityInstallationOptions().rooms.some(
            (secondRoom) => !secondRoom.selected && secondRoom.selectable,
          );
        });
        this.state.pendingFacilityInstallation.roomIds = [];
      }
    ...
    return this.completeUpgradePurchase(upgradeId);
  }

  completeUpgradePurchase(upgradeId, facilityRoomIds = null) {
    const upgrade = this.data.indexes.upgrades[upgradeId];
    if (!upgrade) return false;
    ...
    try {
      ...
      if (upgrade.kind === "FACILITY") {
        this.state.facilityPlacements = {
          ...this.state.facilityPlacements,
          [upgradeId]: { roomIds: [...facilityRoomIds] },
        };
      }
      if (this.isFormalCampaignMode) {
  ...
  facilityInstallationOptions() {
    const pending = this.state.pendingFacilityInstallation;
    if (this.state.phase !== PHASES.UPGRADE || !pending) return null;
    const facility = this.data.indexes.upgrades[pending.upgradeId];
    if (!facility || facility.kind !== "FACILITY" || !facility.installation) return null;
    const board = this.structuralBoardState();
    const selectedRoomIds = [...new Set(pending.roomIds ?? [])];
    const targetCount = facility.installation.target_count;
    const stayoverRoomIds = new Set(
      Object.values(this.state.stayovers).map((entry) => entry.roomId),
    );
    const reservedRoomIds = new Set();
    for (const [facilityId, placement] of Object.entries(this.state.facilityPlacements ?? {})) {
      const installed = this.data.indexes.upgrades[facilityId];
      for (const roomId of placement.roomIds ?? []) {
        if (installed?.installation?.reserves_target) reservedRoomIds.add(roomId);
      }
    }

    const roomBaseReason = (roomId) => {
      const room = board.rooms[roomId];
      if (!room || !board.unlockedRooms.has(roomId)) return "먼저 객실을 증축해야 합니다.";
      if (board.blockedRooms.has(roomId)) {
        return board.blockedReasons.get(roomId) ?? "현재 사용할 수 없는 객실입니다.";
      }
      if (stayoverRoomIds.has(roomId)) return "연박 손님이 사용 중입니다.";
      if (facility.installation.reserves_target && reservedRoomIds.has(roomId)) {
        return "다른 시설의 전용 위치입니다.";
      }
      const missingAttribute = (facility.installation.required_attributes ?? [])
        .find((attribute) => !room.attributes.has(attribute));
      if (missingAttribute) return `${attributeLabel(missingAttribute)} 객실에만 설치할 수 있습니다.`;
      if (selectedRoomIds.length === 1 && !selectedRoomIds.includes(roomId) && targetCount === 2) {
        const firstRoomId = selectedRoomIds[0];
        if (
          facility.installation.relation === "VERTICAL_ADJACENT"
          && !areVerticalAdjacent(firstRoomId, roomId, board)
        ) return "첫 위치의 바로 위나 아래 객실을 선택해야 합니다.";
        if (
          facility.installation.relation === "NON_ADJACENT"
          && (
            areHorizontalAdjacent(firstRoomId, roomId, board)
            || areVerticalAdjacent(firstRoomId, roomId, board)
          )
        ) return "첫 위치와 이웃하지 않은 객실을 선택해야 합니다.";
      }
      return null;
    };
    ...
    const rooms = this.data.rooms.map((room) => {
      ...
      const reason = roomBaseReason(room.id);
      const full = selectedRoomIds.length >= targetCount && !selected;
      return {
        roomId: room.id,
        selected,
        selectable: selected || (!reason && !full),
        reason: full ? "필요한 위치를 모두 선택했습니다." : reason,
      };
    });
    const complete = selectedRoomIds.length === targetCount
      && selectedRoomIds.every((roomId) => !roomBaseReason(roomId))
      && facilityPlacementRelationValid(facility, selectedRoomIds, board);
    return {
      upgradeId: facility.id,
      name: facility.name,
      icon: facility.icon,
      targetCount,
      relation: facility.installation.relation,
      requiredAttributes: (facility.installation.required_attributes ?? []).map((id) => ({
        id,
        label: attributeLabel(id),
      })),
      selectedRoomIds,
      complete,
      effects: (facility.effects ?? []).map((effect) => ({
        type: effect.type,
        scope: effect.scope,
        label: effect.label,
        points: effect.points ?? null,
      })),
      preview: {
        targetRoomIds: selectedRoomIds,
        affectedRoomIds: facilityAffectedRoomIds(facility, selectedRoomIds, board),
        effectLabels: [...new Set((facility.effects ?? []).map((effect) => effect.label))],
      },
      rooms,
    };
  }

  selectFacilityInstallationRoom(roomId) {
    const pending = this.state.pendingFacilityInstallation;
    const options = this.facilityInstallationOptions();
    if (!pending || !options) return false;
    const room = options.rooms.find((candidate) => candidate.roomId === roomId);
    if (!room || !room.selectable) return false;
    if (room.selected) {
      pending.roomIds = pending.roomIds.filter((id) => id !== roomId);
    } else {
      pending.roomIds = [...pending.roomIds, roomId];
    }
    return true;
  }

  confirmFacilityInstallation() {
    const options = this.facilityInstallationOptions();
    if (!options?.complete) return false;
    if (!this.state.currentUpgradeOfferIds.includes(options.upgradeId)) return false;
    const alreadyContracted = this.state.renovationPurchaseIds.some(
      (id) => this.data.indexes.upgrades[id]?.kind === "FACILITY",
    );
    if (alreadyContracted || !this.canContractUpgrade(options.upgradeId)) return false;
    return this.completeUpgradePurchase(options.upgradeId, options.selectedRoomIds);
  }

  cancelFacilityInstallation() {
    if (this.state.phase !== PHASES.UPGRADE || !this.state.pendingFacilityInstallation) return false;
    this.state.pendingFacilityInstallation = null;
    return true;
  }
  ...
  upgradeBlockedByStayover(upgradeId) {
    const upgrade = this.data.indexes.upgrades[upgradeId];
    if (!upgrade) return false;
    if (upgrade.kind === "FACILITY") return false;
    const occupiedRooms = new Set(Object.values(this.state.stayovers).map((entry) => entry.roomId));
    return renovationRoomIds(upgrade, this.state.facilityPlacements)
      .some((roomId) => occupiedRooms.has(roomId));
  }
